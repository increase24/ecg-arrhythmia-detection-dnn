...
def load_all(data_path):
    ...
    for record, label in tqdm.tqdm(records):
        ...
        #print(num_labels)
        dataset.append((ecg_file, label)) # [label]*num_labels
    return dataset 
...
if __name__ == "__main__":
    ...
    valid_frac = 0.2
    data_path = "experiments/cinc17/data/training2017/"
    dataset = load_all(data_path)
    train, dev = split(dataset, valid_frac)
    make_json(os.path.join(this_dir, "train.json"), train)
    make_json(os.path.join(this_dir, "valid.json"), dev)
